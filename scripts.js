...
function SummerHtmlImageMapCreator() {
	...
	var app = (function() {
		...
		return {
			...
			getHTMLCode : function(arg) {
				var html_code = '';
		        var container_height = img.height;
		        ...
		        //var filename = window.localStorage.getItem('fileName');
		          ...
		          if(img.width >= 1180) {
		            html_code += utils.encode('<style>.page-center{width: 1180px;margin:0 auto;}</style><div style="background: url('+ filename +') no-repeat center top;"><div class="page-center"><img src="http://img0.sfht.com/sfhth5/1.1.271/img/spacer.gif" alt="" usemap="#' + time + '" width="100%" height="'+ container_height +'" />') +
		              '<br />' + utils.encode('<map name="' + time + '">') + '<br />';
		            utils.foreachReverse(objects, function(x) {
		              console.log(objects);
		              html_code += '&nbsp;&nbsp;&nbsp;&nbsp;' + utils.encode(x.toString()) + '<br />';
		            });
		            html_code += utils.encode('</map></div></div>');

		          } else {
		            html_code += utils.encode('<img src="'+ filename +'" alt="" usemap="#' + time + '" />') +
		              '<br />' + utils.encode('<map name="' + time + '">') + '<br />';
		            utils.foreachReverse(objects, function(x) {
		              html_code += '&nbsp;&nbsp;&nbsp;&nbsp;' + utils.encode(x.toString()) + '<br />';
		            });
    ...
    href_attr.addEventListener('keyup', function(e) { e.stopPropagation();info.checkDisable(); }, false);
      ...
      checkDisable : function() {
        href_attr.removeAttribute('disabled');
      },
			unload : unload
		};
	})();
	...
	var buttons = (function() {
      ...
      range.selectNode(document.getElementById('code_content'));
    ...
    if (img.width >= 1180){
      this.params.x = this.params.x - (img.width - 1180) / 2
    }
		var x2 = this.params.x + this.params.width,
			y2 = this.params.y + this.params.height;
		return '<area target ="_blank" shape="rect" coords="'
			+ this.params.x + ', '
			+ this.params.y + ', '
			+ x2 + ', '
			+ y2
    ...
    if (img.width >= 1180){
      this.params.cx = this.params.cx - (img.width - 1180) / 2
    }
		return '<area target ="_blank" shape="circle" coords="'
			+ this.params.cx + ', '
			+ this.params.cy + ', '
			+ this.params.radius
			+ '"'
	...
	Polygon.prototype.toString = function() { //to html map area code
      ...
      if (img.width >= 1180 && i % 2 == 0){
        this.params[i] = this.params[i] - (img.width - 1180) / 2
      }
			str += this.params[i];
			if (i != count - 1) {
				str += ', ';
		...
		return '<area target ="_blank" shape="polygon" coords="'
			+ str
			+ '"'
			+ (this.href ? ' href="' + this.href + '"' : '')
			+ (this.alt ? ' alt="' + this.alt + '"' : '')
